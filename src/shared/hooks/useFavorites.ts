/**
 * useFavorites - 즐겨찾기 관리 훅
 * 
 * 주요 역할:
 * 1. 사용자의 즐겨찾기 비디오 목록 조회 및 관리
 * 2. 즐겨찾기 추가/제거 토글 기능
 * 3. React Query를 통한 캐시 관리 및 낙관적 업데이트
 * 4. 로딩 상태 및 에러 처리
 * 
 * 핵심 특징:
 * - Set 자료구조로 빠른 즐겨찾기 조회 성능 제공
 * - 낙관적 업데이트로 즉시 UI 반영
 * - 토글 진행 중 상태 추적으로 중복 요청 방지
 * - 사용자 인증 상태에 따른 조건부 데이터 페칭
 * 
 * 주의사항:
 * - 사용자가 로그인되어 있지 않으면 빈 배열 반환
 * - API 요청 실패 시 이전 상태로 롤백
 * - 동일한 비디오에 대한 연속 토글 요청은 무시
 * - 캐시 무효화를 통한 다른 컴포넌트와의 동기화
 */
'use client'
